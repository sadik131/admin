        ...
        <div className={` mx-auto p-6 shadow-md rounded-lg bg-white gap-5 font-roboto`}>
          ...
          <div className="flex flex-col justify-between gap-4 mb-6">
            <div className='flex w-1/2'>
              <ClassSelect type="Warehouse* " placeholder="Select" padding='py-2 px-4 w-full ' option={[...new Set(categoryData.map(data => data.status))]} />
              <ClassSelect type="Store* " placeholder="Select" padding='py-2 px-4 w-full ' option={[...new Set(categoryData.map(data => data.status))]} />
            </div>
